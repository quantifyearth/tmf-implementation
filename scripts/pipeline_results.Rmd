...
# Rscript -e "rmarkdown::render(input='~/evaluations/R/pipeline_results.Rmd', output_file='~/evaluations/R/example_project.html', params=list(proj='example_project', t0=2010, ...))"
...
if (is.null(params$input_dir) && is.null(params$shapefile)) {
...
} else if(exists(input_dir)) { shapefile_path <- paste0(input_dir,'/',project_name,'.geojson') }
if(exists(shapefile_path)) {shapefile <- read_sf(shapefile_path)}
...
} else if(exists(data_path)) {pairs_path <- file.path(data_path,'pairs') }
...
} else if(exists(data_path)) {carbon_density_path <- list.files(data_path,pattern='carbon',full.names=T)[1] }

if(exists(carbon_density_path)) {
...
} else if(exists(data_path)) {country_path <- list.files(path = data_path, pattern = "country-list", full.names = TRUE)[1]}
...
} else if(exists(data_path)) {additionality_path <- list.files(path = data_path, pattern = "additionality", full.names = TRUE)[1]}
if(exists(additionality_path)) {additionality <- read.csv(additionality_path)}
